...
class Twilio
{
	...
	{		
		...
		if ($resp)
		{
			...
			if (is_object($respxml)) {				
				if ($respxml->SMSMessage->Sid)
				{ 										
					return $respxml->SMSMessage->Sid;			
				} else {
					$this->err=$respxml->RestException->Status;
					$this->err_des=$respxml->RestException->Message;
				}
			} else {
				$this->err=1;
