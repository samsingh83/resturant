...
<a href="<?php echo Yii::app()->request->baseUrl."/admin/";?>" class="uk-button">
...
if ( $res=yii::app()->functions->getOrder($_GET['order_id']) ){		
	...
	$db_ext=new DbExt;
	$db_ext->updateData("{{order}}",array('viewed'=>1),'order_id',$_GET['order_id']);
	...
	$delivery_address=$client_info[0]['delivery_address'];	
	if ( !empty($delivery_address)){
	   if (!preg_match("/$country_name/i", $delivery_address)) {
		   $delivery_address=$delivery_address." ".$country_name;
	   }	   
	   ?>
	   <input type="hidden" name="delivery_address_map" id="delivery_address_map" value="<?php echo $delivery_address?>">
	   <?php
	}	
	?>
	<?php endif;?>
	...
	<div class="print_block">
	  <label><?php echo Yii::t('default',"Card name")?> :</label>
	  <value><?php echo $card_info['card_name']?></value>
	</div>	
	<div class="print_block">
	  <label><?php echo Yii::t('default',"Exp. Month")?> :</label>
	  <value><?php echo $card_info['expiration_month']?></value>
	</div>	
	<div class="print_block">
	  <label><?php echo Yii::t('default',"Exp. Year")?> :</label>
	  <value><?php echo $card_info['expiration_yr']?></value>
	</div>	
	<div class="print_block">
	  <label><?php echo Yii::t('default',"CVV")?> :</label>
	  <value><?php echo $card_info['cvv']?></value>
	</div>	
	<?php 
	$pr_tbl.="<tr>";
	...
	$pr_tbl.="</tr>";
	$pr_tbl.="<tr>";
	$pr_tbl.="<td>".Yii::t("default","Card name")." :</td>";
	$pr_tbl.="<td>".$card_info['card_name']."</td>";
	$pr_tbl.="</tr>";
	$pr_tbl.="<tr>";
	$pr_tbl.="<td>".Yii::t("default","Exp. Month")." :</td>";
	$pr_tbl.="<td>".$card_info['expiration_month']."</td>";
	$pr_tbl.="</tr>";
	$pr_tbl.="<tr>";
	$pr_tbl.="<td>".Yii::t("default","Exp. Year")." :</td>";
	$pr_tbl.="<td>".$card_info['expiration_yr']."</td>";
	$pr_tbl.="</tr>";
	$pr_tbl.="<tr>";
	$pr_tbl.="<td>".Yii::t("default","CVV")." :</td>";
	$pr_tbl.="<td>".$card_info['cvv']."</td>";
	$pr_tbl.="</tr>";
	?>	
	<?php endif;?>	
	...
	<?php if ($trans_type=="delivery"):?>
		...
		<div class="print_block">
		  ...
		  <value><?php echo $client_info[0]['delivery_time']?></value>
		</div>		
		<?php 
		$pr_tbl.="<tr>";
...
?>
    ...
    <a target="_blank" href="<?php echo Yii::app()->request->baseUrl;?>/store/receiptpdf" class="right"><i class="fa fa-cloud-download"></i> <?php echo Yii::t("default","Print to PDF")?></a>
    <div class="clear"></div>
    </div>
    <?php
} else {
