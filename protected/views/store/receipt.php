...
if ( $res=yii::app()->functions->getOrderByUser($_GET['order_id']) ){		
	...
	<?php if ($trans_type=="delivery"):?>
	<?php $delivery_charges=yii::app()->functions->getOption("delivery_charges");?>	
	<?php if ($delivery_charges>0):?>
	<div class="print_block">
	 <p><?php echo Yii::t("default","Delivery charge")?></p>
	 <value><?php echo prettyPrice($delivery_charges);?></value>
	</div>
	<?php 
	$pr_tbl.="<tr>";
	$pr_tbl.="<td>".Yii::t("default","Delivery charge")."</td>";
	$pr_tbl.="<td>".prettyPrice($delivery_charges)."</td>";
	$pr_tbl.="</tr>";
	?>
	<?php endif;?>
	<?php endif;?>
	...
	//$params['status']='paid';
	$params['set_tax']=$tax;
	$params['delivery_charge']=$delivery_charges;
...
if ($receipt_bol==TRUE){
	if (!in_array($_GET['order_id'],(array)$_SESSION['receipt'])){
		
		/*SENT SMS NOTIFICATION */		
		Yii::app()->functions->sendSMSNotification($client_info);
		...
		$subject=yii::app()->functions->getOption('receipt_subject');
		$content=yii::app()->functions->getOption('receipt_content');
		if (!empty($content)){
			$content=Yii::app()->functions->smarty("customer-name",$client_info[0]['fullname'],$content);
			$content=Yii::app()->functions->smarty("receipt-number",$_GET['order_id'],$content);
			$content=Yii::app()->functions->smarty("receipt",$receipt_html,$content);		
		}
		$_SESSION['receipt'][$_GET['order_id']]=$_GET['order_id'];
		...
		if (!empty($client_email)){		
			...
			if ( $email_engine == 2){				
				if (Yii::app()->functions->sendEmailSMTP($client_email,$from,$subject,$content) ){				
			    }
			} else {				
			   if (Yii::app()->functions->sendEmail($client_email,$from,$subject,$content) ){		
			   }
			}
						
		}
		
		$email_address_notify=yii::app()->functions->getOption('email_address_notify');
				
		if (!empty($email_address_notify)){			
			if ( $email_engine == 2){				
				if (Yii::app()->functions->sendEmailSMTP($email_address_notify,$from,$subject,$content) ){				
			    }
			} else {				
			   if (Yii::app()->functions->sendEmail($email_address_notify,$from,$subject,$content) ){		
			   }
			}
		}
	}
