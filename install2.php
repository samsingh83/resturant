...
<link href="<?php echo $host;?>/assets/css/sites.css?ver=1.0" rel="stylesheet" />
<link href="<?php echo $host;?>/assets/css/install.css?ver=1.0" rel="stylesheet" />
  ...
  <div class="main">
    <ul class="order-steps" style="margin-left:150px;">
      <li class="<?php echo $steps==""?"active current":'';?>">
        <a href="<?php echo $host?>/install2.php">Checking Prerequisite</a>  
        <div class="line"></div>
      </li>
...
<div class="page-right-sidebar">
  ...
  <p>main.php file detected. Either you create the main.php manually or you have already install the application.</p>
  <p class="uk-text-danger">Install operation terminated.</p>
  ...
  if ( $failed<=0){
  	  ...
  	  <a class="uk-button uk-button-success uk-width-1-4" href="<?php echo $host;?>/install2.php?step=2">Next</a>
  	  <?php
  } else {
  	  ?>
...
RewriteRule . /$current_dir_folder/index.php [L]
  ...
  $table_prefix=isset($data['db_table_prefix'])?$data['db_table_prefix']:'';
  ...
  $db = new MySQL( $data['db_name'], $data['db_username'], $data['db_pass'],$data['db_host']);
  if (!$db){
  	  echo "<p class=\"uk-text-danger\">ERROR: Cannot connect to database.</p>";
  	  die();
  } 
  ...
  $config_file=str_replace("{table_prefix}",$data['db_table_prefix'],$config_file);  
  ...
  <a class="uk-button uk-button-success uk-width-1-4" href="<?php echo $host;?>/install2.php?step=3">Next</a> 
  ...
  <form class="uk-form uk-form-horizontal forms" id="forms" method="POST">
   ...
   <input type="text" name="db_table_prefix" class="uk-form-width-large" placeholder="rt_" value="<?php echo isset($data['db_table_prefix'])?$data['db_table_prefix']:'rt_'?>" >
   </div>              
  ...
  $data=$_POST;  
  $req=array(
    'store_name'=>"Website title is required",    
  ...
  if ( $validator->validate()){  	  
  	  $admin_currency_set=explode("-",$data['currency_code']);  	    	  
      ...
      $params=array(
       ...
       'currency_code'=>$admin_currency_set[0]
      );   
      ...
      /*insert size data*/      
      $truncate="TRUNCATE TABLE ".$table_prefix."size";   
      $db->raw($truncate);
      if (is_array($size_list) && count($size_list)>=1){
      	  echo "<p class=\"uk-icon-chevron-right\"> Inserting Size  ...</p>";      
      	  foreach ($size_list as $size_data) {      	  	
      	  	$db->create( $table_prefix."size", $size_data); 
      	  }
      	  echo "<div> [done]</div>";  	  
      }      
      ...
      $truncate="TRUNCATE TABLE ".$table_prefix."user";   
      $db->raw($truncate);
      $db->create( $table_prefix."user", $params_user); 
      echo "<div> [done]</div>";
  ...
  <form class="uk-form uk-form-horizontal forms" id="forms" method="POST">
   ...
   <div class="uk-form-row">                           
   <label class="uk-form-label">Store Name</label>
   <input type="text" name="store_name" class="uk-form-width-large" value="<?php echo isset($data['store_name'])?$data['store_name']:""?>" >
   </div>      
   ...
   <input type="text" name="phone_number" class="uk-form-width-large" value="<?php echo isset($data['phone_number'])?$data['phone_number']:""?>" >
   </div>      
   
   <div class="uk-form-row">                           
   <label class="uk-form-label">Contact Email</label>
   <input type="text" name="contact_email" class="uk-form-width-large" value="<?php echo isset($data['contact_email'])?$data['contact_email']:""?>" >
   </div>      
   
   <div class="uk-form-row">                           
   <label class="uk-form-label">Currency Code</label>
   <?php echo $forms->dropDownList('currency_code','',$currency_list)?>
